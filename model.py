...
class ConcreteDropout(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.init_drop_p = init_p
        if init_p:
            self.log_drop_p = nn.Parameter(
                torch.log(torch.tensor(self.init_drop_p, dtype=torch.float32))
            )
        else:
    ...
    def _concrete_dropout(self, x: torch.Tensor, drop_prob: float) -> torch.Tensor:
        retain_p = 1 - drop_prob
        eps = 1e-7
        u = torch.rand_like(x)
        # mask contains continuous values in [0, 1] for each neuron but very close to 0 or 1
        drop_prob_mask = torch.sigmoid(
            (
                torch.log(drop_prob + eps)
                - torch.log(retain_p + eps)
                + torch.log(u + eps)
                - torch.log(1 - u + eps)
            )
            / self.temperature
        ...
        output = (
            x * retain_prob_mask / retain_p
        )  # Scaling output by retain_p to maintain expected value
        return output

    # TODO: explore doing loss calculation outside this class
    def forward(self, x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        drop_prob = torch.sigmoid(self.log_drop_p)
        retain_prob = 1 - drop_prob
        if self.training:
            output = self._concrete_dropout(x, drop_prob)
            eps = 1e-7
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            nats = F.cross_entropy(logits, targets, reduction="mean")  # Average loss

            # Convert nats to bits (BPC)
            bpc = nats / torch.log(torch.tensor(2.0))
            try:
                dropout_loss = sum(
        ...
        return logits, loss, dropout_loss, bpc
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        flops_promised = 83e12  # rtx 4090
        # flops_promised = 2e12 # estimated macbook air
